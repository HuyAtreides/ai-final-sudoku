...
class SudokuGame:
    def __init__(self):
        ...
        ]  # 0: original, 1: user input, 2: hint
    ...
    def initialize_sounds(self):
        ...
        self.button_sound = pygame.mixer.Sound(
            self.create_beep_sound(440, 100)
        )  # A4 note
        self.error_sound = pygame.mixer.Sound(
            self.create_beep_sound(220, 300)
        )  # A3 note
        self.success_sound = pygame.mixer.Sound(
            self.create_beep_sound(880, 200)
        )  # A5 note
    ...
    def create_beep_sound(self, frequency, duration):
        ...
        for i in range(0, int(sample_rate * duration / 1000)):
            sample = (
                0.5 * max_sample * math.sin(2 * math.pi * frequency * i / sample_rate)
            )
    ...
    def generate_board(self):
        ...
        # Reset cell status
        self.cell_status = [
            [0 if self.board[i][j] != 0 else 1 for j in range(9)] for i in range(9)
        ]
    ...
    def draw_welcome_screen(self):
        ...
        buttons.append(
            self.create_button(
                "NEW GAME", WIDTH // 2, HEIGHT // 2 - 60, self.start_new_game
            )
        )

        if self.has_saved_game:
            buttons.append(
                self.create_button("CONTINUE", WIDTH // 2, HEIGHT // 2, self.load_game)
            )
        else:
            # Create a disabled button
            disabled_button = self.create_button(
                "CONTINUE", WIDTH // 2, HEIGHT // 2, None
            )
            buttons.append(disabled_button)

        buttons.append(
            self.create_button(
                "SETTINGS", WIDTH // 2, HEIGHT // 2 + 60, self.open_settings
            )
        )
        buttons.append(
            self.create_button(
                "HIGH SCORES", WIDTH // 2, HEIGHT // 2 + 120, self.open_high_scores
            )
        )
        buttons.append(
            self.create_button("QUIT", WIDTH // 2, HEIGHT // 2 + 180, self.confirm_quit)
        )
    ...
    def draw_board(self):
        ...
        pygame.draw.rect(
            self.screen, LIGHT_GRAY, (offset_x, offset_y, BOARD_SIZE, BOARD_SIZE)
        )
        ...
        for i in range(9):
            for j in range(9):
                ...
                pygame.draw.rect(
                    self.screen, cell_color, (cell_x, cell_y, GRID_SIZE, GRID_SIZE)
                )
                ...
                if self.board[i][j] != 0:
                    ...
                    num_text = self.large_font.render(
                        str(self.board[i][j]), True, num_color
                    )
                    num_rect = num_text.get_rect(
                        center=(cell_x + GRID_SIZE // 2, cell_y + GRID_SIZE // 2)
                    )
        ...
        for i in range(10):
            ...
            pygame.draw.line(
                self.screen,
                self.current_colors["grid"],
                (offset_x + i * GRID_SIZE, offset_y),
                (offset_x + i * GRID_SIZE, offset_y + BOARD_SIZE),
                line_width,
            )
            ...
            pygame.draw.line(
                ...
                (offset_x + BOARD_SIZE, offset_y + i * GRID_SIZE),
                line_width,
            )
    ...
    def draw_game_screen(self):
        ...
        buttons.append(
            self.create_button("MARK", WIDTH // 6 * 2, button_y, self.toggle_mark)
        )
        buttons.append(
            self.create_button("CLEAR", WIDTH // 6 * 3, button_y, self.clear_board)
        )
        buttons.append(
            self.create_button("SAVE", WIDTH // 6 * 4, button_y, self.save_game)
        )
        buttons.append(
            self.create_button("SOLVE", WIDTH // 6 * 5, button_y, self.select_algorithm)
        )
    ...
    def draw_game_info(self):
        # self.screen.blit(back,(0,0))
        # Draw difficulty
        diff_text = self.medium_font.render(
            f"Difficulty: {self.difficulty.value['name']}",
            True,
            self.current_colors["text"],
        )
        ...
        time_text = self.medium_font.render(
            f"Time: {time_str}", True, self.current_colors["text"]
        )
        self.screen.blit(time_text, (WIDTH - 150, 20))

        # Draw hints used
        hint_text = self.medium_font.render(
            f"Hints: {self.hints_used}", True, self.current_colors["text"]
        )
        ...
        if self.algo_solve_time > 0:
            algo_text = self.small_font.render(
                f"Solve time: {self.algo_solve_time:.6f}s",
                True,
                self.current_colors["text"],
            )
    ...
    def draw_settings_screen(self):
        ...
        title_text = self.large_font.render(
            "SETTINGS", True, self.current_colors["text"]
        )
        ...
        diff_text = self.medium_font.render(
            "Difficulty:", True, self.current_colors["text"]
        )
        ...
        # Draw theme setting
        theme_text = self.medium_font.render(
            "Theme:", True, self.current_colors["text"]
        )
        ...
        # Draw volume setting
        vol_text = self.medium_font.render(
            f"Volume: {int(self.sound_volume * 100)}%",
            True,
            self.current_colors["text"],
        )
        ...
        pygame.draw.circle(
            self.screen, self.current_colors["button"], (handle_x, slider_y + 5), 10
        )
    ...
    def draw_high_scores_screen(self):
        ...
        # Draw title
        title_text = self.large_font.render(
            "HIGH SCORES", True, self.current_colors["text"]
        )
        title_rect = title_text.get_rect(center=(WIDTH // 2, 60))
        self.screen.blit(title_text, title_rect)
        ...
        if sorted_scores:
            for i, score in enumerate(sorted_scores[:10]):
                ...
                # Draw rank
                rank_text = self.medium_font.render(
                    f"{i+1}.", True, self.current_colors["text"]
                )
                self.screen.blit(rank_text, (WIDTH // 4 - 80, y_pos))

                # Draw name
                name_text = self.medium_font.render(
                    name, True, self.current_colors["text"]
                )
                self.screen.blit(name_text, (WIDTH // 4, y_pos))

                # Draw difficulty
                diff_text = self.medium_font.render(
                    diff_name, True, self.current_colors["text"]
                )
                self.screen.blit(diff_text, (WIDTH // 2 - 50, y_pos))

                # Draw time
                time_text = self.medium_font.render(
                    time_str, True, self.current_colors["text"]
                )
                self.screen.blit(time_text, (WIDTH // 2 + 60, y_pos))

                # Draw hints
                hints_text = self.medium_font.render(
                    f"Hints: {hints}", True, self.current_colors["text"]
                )
                self.screen.blit(hints_text, (WIDTH * 3 // 4, y_pos))
        else:
            # Display message if no high scores
            no_scores_text = self.medium_font.render(
                "No high scores yet!", True, self.current_colors["text"]
            )
            no_scores_rect = no_scores_text.get_rect(center=(WIDTH // 2, HEIGHT // 2))
            self.screen.blit(no_scores_text, no_scores_rect)
    ...
    def draw_quit_confirm_screen(self):
        ...
        quit_text = self.large_font.render(
            "Are you sure you want to quit?", True, self.current_colors["text"]
        )
        quit_rect = quit_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 50))
        self.screen.blit(quit_text, quit_rect)

        # Draw buttons
        yes_button = self.create_button(
            "YES", WIDTH // 2 - 75, HEIGHT // 2 + 50, self.quit_game
        )
        no_button = self.create_button(
            "NO", WIDTH // 2 + 75, HEIGHT // 2 + 50, self.go_back
        )
    ...
    def draw_algorithm_select_screen(self):
        ...
        # Draw title
        title_text = self.large_font.render(
            "SELECT SOLVING ALGORITHM", True, self.current_colors["text"]
        )
        title_rect = title_text.get_rect(center=(WIDTH // 2, HEIGHT // 3 - 50))
        self.screen.blit(title_text, title_rect)

        # Draw buttons
        backtrack_button = self.create_button(
            "Backtracking",
            WIDTH // 2,
            HEIGHT // 2 - 30,
            lambda: self.solve_puzzle("backtracking"),
        )
        constraint_button = self.create_button(
        ...
        back_button = self.create_button(
            "BACK", WIDTH // 2, HEIGHT // 2 + 90, self.go_back
        )
            ...
            for num in range(1, 10):
                if self.is_valid_in_board(temp_board, row, col, num):
                    ...
                    if hasattr(self, "visualize_solving") and self.visualize_solving:
                        ...
                        pygame.time.delay(75)  # Add a small delay to see the progress
                    ...
                    if (
                        hasattr(self, "visualize_solving")
                        and self.visualize_solving
                        and not self.stop_solving
                        and not self.skip_to_solution
                    ):
                        self.board = copy.deepcopy(temp_board)
                        self.active_cell = (row, col)
                        self.draw_game_screen()
                        ...
                        # Process events to check for stop/skip button presses
                        self.process_solving_events()
                        pygame.time.delay(75)  # Shorter delay for backtracking
            ...
            for value in list(possible_values[min_cell]):
                ...
                if self.is_valid_in_board(temp_board, row, col, value):
                    ...
                    if hasattr(self, "visualize_solving") and self.visualize_solving:
                        ...
                        self.draw_game_screen()
                        pygame.display.flip()
                        pygame.time.delay(75)  # Add delay to see progress
                    ...
                    for i in range(9):
                        # Row cells
                        if (i, col) in possible_values and value in possible_values[
                            (i, col)
                        ]:
                        ...
                        # Column cells
                        if (row, i) in possible_values and value in possible_values[
                            (row, i)
                        ]:
                    ...
                    for i in range(box_row, box_row + 3):
                        for j in range(box_col, box_col + 3):
                            if (i, j) in possible_values and value in possible_values[
                                (i, j)
                            ]:
    ...
    def input_player_name(self):
        ...
        while input_active:
            ...
            # Render the messages
            congrats_surf = self.medium_font.render(
                congrats_message, True, self.current_colors["text"]
            )
            name_prompt_surf = self.medium_font.render(
                name_prompt, True, self.current_colors["text"]
            )
            player_name_surf = self.medium_font.render(
                player_name, True, self.current_colors["text"]
            )
            ...
            name_prompt_rect = name_prompt_surf.get_rect(
                center=(WIDTH // 2, HEIGHT // 2 + 20)
            )
            player_name_rect = player_name_surf.get_rect(
    ...
    def ask_play_again(self):
        ...
        # Draw confirmation message
        question_text = self.large_font.render(
            "Do you want to play again?", True, self.current_colors["text"]
        )
        ...
        yes_button = self.create_button(
            "YES", WIDTH // 2 - 75, HEIGHT // 2 + 50, self.restart_game
        )  # Restart game function
        no_button = self.create_button(
            "NO", WIDTH // 2 + 75, HEIGHT // 2 + 50, self.go_to_main_menu
        )  # Main menu function
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:  # Left click
                    ...
                    ) and self.stop_button_rect.collidepoint(mouse_pos):
                        self.play_sound(self.button_sound)
                    ...
                    ) and self.skip_button_rect.collidepoint(mouse_pos):
                        self.play_sound(self.button_sound)
